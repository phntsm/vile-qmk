...
void matrix_scan_user(void) { // The very important timer.
    ...
    const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {
        ...
        [_FN4]  =  { ENCODER_CCW_CW(KC_VOLD,  KC_VOLU), ENCODER_CCW_CW(RGB_RMOD, RGB_MOD), ENCODER_CCW_CW(KC_VOLD,  KC_VOLU) },
        [_FN5]  =  { ENCODER_CCW_CW(KC_VOLD,  KC_VOLU), ENCODER_CCW_CW(RGB_RMOD, RGB_MOD), ENCODER_CCW_CW(KC_VOLD,  KC_VOLU) },
        [_FN6]  =  { ENCODER_CCW_CW(KC_VOLD,  KC_VOLU), ENCODER_CCW_CW(RGB_RMOD, RGB_MOD), ENCODER_CCW_CW(KC_VOLD,  KC_VOLU) },
        [_FN7]  =  { ENCODER_CCW_CW(KC_VOLD,  KC_VOLU), ENCODER_CCW_CW(RGB_RMOD, RGB_MOD), ENCODER_CCW_CW(KC_VOLD,  KC_VOLU) },
        [_FN8]  =  { ENCODER_CCW_CW(KC_VOLD,  KC_VOLU), ENCODER_CCW_CW(RGB_RMOD, RGB_MOD), ENCODER_CCW_CW(KC_VOLD,  KC_VOLU) },
    };
#endif
...
//LOCK STATUS
    void render_keylock_status(led_t led_state) {
        ...
        oled_write(PSTR("CAP"), caps_state);
        oled_write(PSTR(" "), false);
        oled_write(PSTR("NUM"), led_state.num_lock);
        oled_write(PSTR(" "), false);
        oled_write(PSTR("SCR"), led_state.scroll_lock);
...
// LAYER STATUS
    void render_current_layer(){
        switch (get_highest_layer(layer_state)) {
                case 0:
                    oled_write(PSTR("Foundation"), false);
                    break;
                case 1:
                    oled_write(PSTR("Legion"), false);
                    break;
                case 2:
                    oled_write(PSTR("Gizmos"), false);
                    break;
                case 3:
                    oled_write(PSTR("Malfunction"), false);
                    break;
                case 4:
                    oled_write(PSTR("Wanderlust"), false);
                    break;
                case 5:
                    oled_write(PSTR("Pentagram"), false);
                    break;
                case 6:
                    oled_write(PSTR("Spectre"), false);
                    break;
                case 7:
                    oled_write(PSTR("Spectrum"), false);
                    break;
                case 8:
                    oled_write(PSTR("Lament CFG"), false);
                    break;
                default:
                    oled_write(PSTR("Z?"), false);    // Should never display, here as a catchall
        }
    }
// WPM
    void render_current_wpm(){
        oled_write_P(PSTR("Toll "), false);
